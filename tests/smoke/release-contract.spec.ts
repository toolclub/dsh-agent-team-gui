...
describe('release engineering contract', () => {
  it('declares a valid v0.5+ bundle and official locale dependency', () => {
    ...
    expect(manifest.peerDependencies['@deepseek-ai/dsh-client-locale']).toBe('>=0.1.5-rc.1 <0.1.6-0')
    expect(manifest.peerDependencies['@deepseek-ai/dsh-client-ui-slots']).toBe('>=0.1.5-rc.1 <0.1.6-0')
    expect(manifest.peerDependencies['@deepseek-ai/dsh-jobs']).toBe('>=0.1.5-rc.1 <0.1.6-0')
  ...
  it('makes API v5 and the hermetic DSH boundary explicit', () => {
    ...
    expect(common).toContain('DEFAULT_API_VERSION = 5')
    ...
    expect(browser).toContain("isAgentTeamResponse(candidate, 'mode/next-set')")
    ...
    expect(browser).toContain("isAgentTeamResponse(candidate, 'project/default-set')")
